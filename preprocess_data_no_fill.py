...
# Load the dataset
df = pd.read_csv('data/by_country/afg.csv')
...
# I've removed the MLE imputation section here

# Identify constant columns (columns with only one unique value)
constant_columns = []
for col in numeric_cols:
    if df_no_duplicates[col].nunique() <= 2:
        constant_columns.append(col)

print(f"Found {len(constant_columns)} constant columns")

# Remove constant columns
df_without_constants = df_no_duplicates.drop(columns=constant_columns)
