...
def perform_causal_analysis(data_file, output_dir="output"):
    """
    Perform causal discovery using causallearn's PC algorithm on all variables
    ...
    edge_file = os.path.join(output_dir, "pc_edges.txt")
    with open(edge_file, 'w') as f:
        f.write("Causal edges discovered by PC:\n")
    ...
    graph_file = os.path.join(output_dir, "pc_causal_graph.png")
    plt.title("Causal Graph from PC Algorithm", fontsize=16)
    plt.tight_layout()
    plt.savefig(graph_file, dpi=300, bbox_inches='tight')
    print(f"\nSaved causal graph to {graph_file}")
    
    svg_file = os.path.join(output_dir, "pc_causal_graph.svg")
